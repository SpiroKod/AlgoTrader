# -*- coding: utf-8 -*-
"""
Created on Sun Mar 28 11:09:36 2021
...
for t in tickers:
    ohlcv_intraday[t]=yf.download(t,start,stop,interval="15m")
...
for ticker in tickers:
    ...
    for i in range(1,len(ohlc_dict[ticker])):
        if tickers_signal[ticker] == "":
            ...
            if (ohlc_dict[ticker]["Low"][i]<ohlc_dict[ticker]["Close"][i-1] - ohlc_dict[ticker]["ATR"][i-1]) or (open_ret>=0.05):
               ...
               ohlc_dict[ticker]["Volume"][i]>1.5*ohlc_dict[ticker]["roll_max_vol"][i-1]:
                ...
                tickers_ret[ticker].append((ohlc_dict[ticker]["Close"][i]/ohlc_dict[ticker]["Close"][i-1])-1)
                open_ret=open_ret+ tickers_ret[ticker][-1]
                
        elif tickers_signal[ticker] == "Sell":
            if (ohlc_dict[ticker]["High"][i]>ohlc_dict[ticker]["Close"][i-1] + ohlc_dict[ticker]["ATR"][i-1]) or (open_ret>=0.05):
                tickers_signal[ticker] = ""
                ohlc_dict[ticker]["Signal"][i]="Stop Loss Triggered"
                open_ret=0
...
for i in range(len(KPI_df)):
    if KPI_df["Return"][i]<0:
        print(KPI_df.index[i],KPI_df["Return"][i])
        n=n+1